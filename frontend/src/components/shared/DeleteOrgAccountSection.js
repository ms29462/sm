...
const DeleteOrgAccountSection = () => {
  ...
  const handleRequest = async () => {
    setLoading(true);
    try {
      await api.requestClubDeletion({ reason });
  ...
  const handleCancel = async () => {
    try {
      await api.cancelClubDeletionRequest();
  ...
  return (
    <div className="bg-card border border-red-500/20 rounded-sm p-6 mt-6">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Requesting account deletion will permanently remove your profile, opportunities, applications,
        and subscription once processed by our team. This action cannot be undone.
      </p>
...
export default DeleteOrgAccountSection;