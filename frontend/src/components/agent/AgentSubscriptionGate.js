...
const AgentSubscriptionGate = ({ children }) => {
  const [status, setStatus] = useState(null);
  const [loading, setLoading] = useState(true);
  const navigate = useNavigate();

  useEffect(() => {
    api.getAgentSubscriptionStatus()
    ...
    return (
      <div className="p-4 md:p-8 flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <p className="text-sm text-muted-foreground max-w-md mb-6">
          Your profile has been approved. Please activate your subscription to access Soccer Match.
        </p>
        <button onClick={() => navigate("/agent/subscribe")}
...
export default AgentSubscriptionGate;