...
const AcademyCredits = () => {
  ...
  const handleBuyPack = async (packId) => {
    try {
      const res = await api.createAcademyCheckout(packId);
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="bg-card border border-border/50 rounded-sm p-4 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">Total Purchased</p>
            </div>
            <div className="bg-card border border-border/50 rounded-sm p-4 text-center">
          ...
          <div className="bg-card border border-border/50 rounded-sm p-4">
            ...
            <div className="space-y-2 text-sm text-muted-foreground">
              <p>⭐ Credits are required to submit player applications to opportunities</p>
              <p>💳 Buy credit packs for instant access — credits never expire</p>
              <p>🔄 Credits may be refunded if an opportunity is removed by admin</p>
            </div>
          </div>
        </div>
      )}
