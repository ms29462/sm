...
const CollegeSubscriptionGate = ({ children }) => {
  const [status, setStatus] = useState(null);
  const [loading, setLoading] = useState(true);
  const navigate = useNavigate();

  useEffect(() => {
    api.getCollegeSubscriptionStatus()
    ...
    return (
      <div className="p-4 md:p-8 flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <p className="text-sm text-muted-foreground max-w-md mb-6">
          Your institution has been approved. Please activate your subscription to access Soccer Match.
        </p>
...
export default CollegeSubscriptionGate;