...
const SpecialistHome = () => {
  ...
  const loadData = async () => {
    try {
      const [profileRes, clientsRes] = await Promise.all([
        api.getSpecialistProfile(),
        api.getSpecialistFavorites().catch(() => ({ data: [] }))
      ]);
  ...
  return (
    <div className="p-8" data-testid="specialist-dashboard">
      <div className="mb-8">
      ...
      {profile?.approved && !profile?.specialist_type && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-card border border-border/50 p-6 rounded-sm">
          ...
          <Link to="/specialist/clients" className="text-primary text-sm hover:underline">View all</Link>
        </div>
        ...
        <div className="bg-card border border-border/50 p-6 rounded-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm font-medium text-muted-foreground">
              {profile?.specialist_type || 'Not Set'}
            </span>
          </div>
          <h3 className="font-heading uppercase text-sm text-muted-foreground">Specialization</h3>
          <Link to="/specialist/profile" className="text-primary text-sm hover:underline">Edit profile</Link>
        </div>
        ...
        <div className="bg-card border border-border/50 p-6 rounded-sm">
          ...
          <h3 className="font-heading uppercase text-sm text-muted-foreground">
            {profile?.verified ? 'Verified Specialist' : 'Verification Pending'}
          </h3>
