...
const SpecialistPlayerDetail = () => {
  ...
  const loadPlayer = async () => {
    try {
      const response = await api.getSpecialistPlayerDetail(playerId);
      setPlayer(response.data);
    } catch (error) {
      toast.error('Failed to load player');
      navigate('/specialist/players');
    } finally {
      setLoading(false);
    }
  };
  ...
  const toggleFavorite = async () => {
    try {
      if (isFavorite) {
        ...
        await api.addSpecialistFavorite(playerId);
        setIsFavorite(true);
  ...
  return (
    <div className="p-8" data-testid="specialist-player-detail">
      <button
        onClick={() => navigate(-1)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Physical Stats - Important for Specialists */}
          <div className="bg-card border border-border/50 p-6 rounded-sm">
...
export default SpecialistPlayerDetail;
