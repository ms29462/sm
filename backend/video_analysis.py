...

{
  ...
  "technical_skills": {
    ...
    "shooting": <1-100>,
    "dribbling": <1-100>,
    "heading": <1-100 or null if not shown>,
    "first_touch": <1-100>,
...
async def download_video_from_url(url: str, output_path: str) -> bool:
    ...
    try:
        # Check if it's a direct video URL first (mp4, webm, etc.)
        ...
        # Check if it's a YouTube or Vimeo URL - try yt-dlp
        youtube_patterns = [
...
async def analyze_video_with_gemini(video_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        chat = LlmChat(
            ...
            session_id=f"video-analysis-{uuid.uuid4()}",
            system_message="You are an expert football scout and video analyst. Always respond with valid JSON only, no additional text."
...
async def analyze_highlight_video(video_url: str, player_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not download_success or not os.path.exists(video_path):
            return {
                "success": False,
                "error": "Failed to download video. Please ensure the URL is valid and accessible.",
                "analysis_id": analysis_id
            }
        
        # Check file size (limit to 50MB for API constraints)
        file_size = os.path.getsize(video_path)
        if file_size > 50 * 1024 * 1024:
            return {
                "success": False,
                "error": "Video file is too large (>50MB). Please use a shorter highlight clip.",
                "analysis_id": analysis_id
            }
